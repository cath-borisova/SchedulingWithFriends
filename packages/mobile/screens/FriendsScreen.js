...
export default function HomeScreen() {
  const personA = false; //A is Julia B is Daniel
  let friends = [];
  let friendImages = []

  if (personA) {
     friends = ['Anna Bamtise', 'Katya Borisova', 'Gregor Wuend', 'Evyn Sietsema', 'Max Brooksen', 'Jessica Peterson'];
  ...
  const addFriend = () => {
    setAddFriends(false);
    if (personA) {
      setDisplay(['Anna Bamtise', 'Katya Borisova', 'Gregor Wuend', 'Evyn Sietsema', 'Max Brooksen', 'Jessica Peterson', 'Daniel Dowdle']);
    } else {
      setDisplay(['Julia Truong']);
    }
        ...
        {addFriends &&
          <View style={styles.overlay}>
            ...
            <View style={styles.row2}>
              <Pressable style={{ flex:1, marginBottom: 15, alignSelf: 'flex-start'}}><Text style={{fontSize: normalizeHeight(26), alignSelf: 'center'}}>Add</Text></Pressable>
